...
public class ImageCropper : MonoBehaviour
{
	public class Settings
	{
		...
		public float selectionInitialPaddingLeft = 0.0f;
		public float selectionInitialPaddingTop = 0.0f;
		public float selectionInitialPaddingRight = 0.0f;
		public float selectionInitialPaddingBottom = 0.0f;
	}
...
#pragma warning restore 0649
	...
	public void Show( Texture image, CropResult onCrop, Settings settings = null, ImageResizePolicy croppedImageResizePolicy = null )
	{
		...
		UpdateSelection( Vector2.zero, m_orientedImageSize.ScaleWith( paddingMax) );
		m_selection.anchoredPosition = ( m_orientedImageSize - m_selection.sizeDelta ) * 0.5f;
		if( m_pixelPerfectSelection )
			MakePixelPerfectSelection();

#if UNITY_ANDROID
        if (FindObjectOfType<DemoScene>().m_Mode == 1)
        {
            RotateClockwise(1);
        }
#endif
	...
	public void Hide()
	{
        FindObjectOfType<DemoScene>().PhotoPanel.SetActive(true);
        //FindObjectOfType<DemoScene>().pickPreiveimage.sprite = null;
	...
	public Texture2D CropSelection( int width, int height )
	{
		...
		try
		{
            ...
            cropRenderCamera.backgroundColor = Color.black;
...
/*
		...
		if( aspectRatio < 1 )
		{
			if( shrinkToFit )
			{
				float requiredHeight = newSize.x / 1;
				if( requiredHeight >= currMinSize.y )
					newSize.y = requiredHeight;
				else
				{
					float requiredWidth = currMinSize.y * 1;
			...
			{
				float requiredWidth = newSize.y * 1;
				if( requiredWidth <= currMaxSize.x )
					newSize.x = requiredWidth;
				else
				{
					float requiredHeight = currMaxSize.x / 1;
		...
		else if( aspectRatio > 1 )
		{
			if( shrinkToFit )
			{
				...
				{
					float requiredHeight = currMinSize.x / 1;
					if( requiredHeight <= currMaxSize.y )
			...
			else
			{
				...
				{
					float requiredWidth = currMaxSize.y * 1;
					if( requiredWidth <= currMaxSize.x )
