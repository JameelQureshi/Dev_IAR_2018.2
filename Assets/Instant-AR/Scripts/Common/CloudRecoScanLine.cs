...
public class CloudRecoScanLine : MonoBehaviour
{
    ...
    void Update()
    {
        CloudEnabled = !GlobalVariables.TRACKING_FOUND;
        ...
        if (CloudEnabled)
        {
            ...
            // Get the main camera
            //Debug.Log("<color=red> ??????? Ohhh m_Camera is: !  </color>"+ m_Camera);
            if (m_Camera == null){
                Start();
                //Debug.Log("<color=green> ??????? BUT NOW  m_Camera is: !  </color>" + m_Camera);
            }
